...
exports.getResRaw = function(req, res) {
    ...
    if (fs.existsSync(filepath)) {
        ...
        res.send('cache files had been removed, please refresh target page to capture again!');
        res.end();
    }
};
...
// clone proj
exports.poolClone = function(req, res) {
    var idx = req.param('idx');
    var r = _.find(config.pool.children, function(proj) {
        return proj.id == idx;
    });
    if (r) {
        var proj = _.clone(r);
        proj.name = req.param('name');
        proj.match = req.param('match');
        config.pool.children.push(proj);
        removeId(config.pool);
        // disable require cache
        delete require.cache[path.resolve(__dirname, '../../../', 'rules/pool.js')];
        config.saveRules(function() {
            if (liveapp.socket) {
                liveapp.socket.emit('poolTreeUpdated');
            }
        });
        res.json({
            code: 0
        ...
        switch (key) {
            ...
            case 'speedLimit':
                if (value == 'Disabled') {
                    ...
                    util.speedLimitStart(value, config.global.http);
                }
                break;
            default:
                break;
...
exports.compose = function(req, res) {
    ...
    var headers = JSON.parse(req.param('headers') || '{}');
...
exports.ruleImport = function(req, res) {
    ...
    config.saveRules();
    socket && socket.emit('rulesImport');
    res.json({
        code: 0
    });
};
...
module.exports = function(liveapp) {
    ...
    liveapp.post('/pool/clone', that.poolClone);
